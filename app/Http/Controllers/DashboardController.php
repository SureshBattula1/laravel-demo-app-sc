...
class DashboardController extends Controller
{
    ...
    public function getSuperAdminOverview(Request $request)
    {
        try {
            ...
            $branchIds = $this->getBranchIdsForCompany($user->company_id ?? 0);
            ...
            // Get teachers and students count
            ...
            // Get financial data
            $financialData = DB::table('transactions')
                ->where('status', 'Approved')
                ->whereIn('branch_id', $branchIds)
                ->select(
            ...
            $totalRevenue = (float) ($financialData->total_revenue ?? 0);
            $totalExpenses = (float) ($financialData->total_expenses ?? 0);
            $netBalance = $totalRevenue - $totalExpenses;

            // Get branch breakdown
            $branchBreakdown = DB::table('branches as b')
                ->leftJoin('teachers as t', 'b.id', '=', 't.branch_id')
                ->leftJoin('students as s', 'b.id', '=', 's.branch_id')
                ->whereIn('b.id', $branchIds)
                ->select(
                    'b.id as branchId',
                    'b.name as branchName',
                    DB::raw('COUNT(DISTINCT CASE WHEN t.teacher_status = "Active" THEN t.id END) as teacherCount'),
                    DB::raw('COUNT(DISTINCT CASE WHEN s.student_status = "Active" THEN s.id END) as studentCount'),
                    DB::raw('0 as revenue')
                )
                ->groupBy('b.id', 'b.name')
                ->get()
            ...
            $branchRevenue = DB::table('transactions as tr')
                ->where('status', 'Approved')
                ->where('type', 'Income')
                ->whereIn('branch_id', $branchIds)
                ->select(
            ...
            foreach ($branchBreakdown as $branch) {
                $branch->revenue = (float) ($branchRevenue->get($branch->branchId)->revenue ?? 0);
            }
            ...
            Log::error('SuperAdmin Overview Error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
     ...
     */
    public function getAdminOverview(Request $request, $branchId)
    {
        try {
            $user = $request->user();

            // Allow SuperAdmin (all branches) and BranchAdmin (their branch only)
            ...
            // Verify BranchAdmin has access to this specific branch
            if ($user->role === 'BranchAdmin' && (int) $user->branch_id !== (int) $branchId) {
                return response()->json([
                    'success' => false,
            ...
            // SuperAdmin can access any branch without restriction
            // (they have no branch_id assigned in the users table)
            ...
            $todayAttendance = DB::table('student_attendance')
                ->where('branch_id', $branchId)
                ->whereDate('date', Carbon::today())
                ->select(
                    DB::raw('SUM(CASE WHEN status = "Present" THEN 1 ELSE 0 END) as present'),
            ...
            if ($todayAttendance && $todayAttendance->total > 0) {
                $attendancePercentage = round(($todayAttendance->present / $todayAttendance->total) * 100);
            }
     ...
     */
    public function getTeacherOverview(Request $request)
    {
        try {
            $user = $request->user();
            ...
            // Get total teachers and students in branch
            $totalTeachers = DB::table('teachers')
                ->where('branch_id', $branchId)
                ->where('teacher_status', 'Active')
                ->count();
            ...
            $todayStudentAttendance = DB::table('student_attendance')
                ->where('branch_id', $branchId)
                ->whereDate('date', Carbon::today())
                ->select(
                    DB::raw('SUM(CASE WHEN status = "Present" THEN 1 ELSE 0 END) as present'),
            ...
            $presentToday = (int) ($todayStudentAttendance->present ?? 0);
            $absentToday = (int) ($todayStudentAttendance->absent ?? 0);
            ...
            Log::error('Teacher Overview Error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Error loading overview data'
            ], 500);
    ...
    public function getStudentOverview(Request $request)
    {
        try {
            ...
            Log::error('Student Overview Error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Error loading overview data'
            ], 500);
