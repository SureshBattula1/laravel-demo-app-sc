...
class SmsBulkRecipientSeedService
{
    ...
    public function repairFromQueuedJobPayloads(SmsBulkQueue $queue, int $branchId): bool
    {
        ...
        foreach (['jobs', 'failed_jobs'] as $table) {
            ...
            DB::table($table)
                ->select('payload')
                ->where(function ($q) {
                    $q->where('payload', 'like', '%ProcessBulkSmsChunkJob%')
                        ->orWhere('payload', 'like', '%ProcessBulkWhatsAppChunkJob%');
                })
                ->orderBy('id')
                ->chunk(200, function ($rows) use ($queue, $branchId, &$studentIds, &$teacherIds) {
                    foreach ($rows as $row) {
                        ...
                        $job = $this->unserializeAnyChunkJob((string) $payload['data']['command']);
                        if (! $job instanceof ProcessBulkSmsChunkJob && ! $job instanceof ProcessBulkWhatsAppChunkJob) {
                            continue;
                        }
    ...
    private function unserializeAnyChunkJob(string $raw): ProcessBulkSmsChunkJob|ProcessBulkWhatsAppChunkJob|null
    {
        $allowed = [ProcessBulkSmsChunkJob::class, ProcessBulkWhatsAppChunkJob::class];
        $obj = @unserialize($raw, ['allowed_classes' => $allowed]);
        if ($obj instanceof ProcessBulkSmsChunkJob || $obj instanceof ProcessBulkWhatsAppChunkJob) {
            return $obj;
        }

        try {
            $decrypted = Crypt::decryptString($raw);
            $obj = @unserialize($decrypted, ['allowed_classes' => $allowed]);
            if ($obj instanceof ProcessBulkSmsChunkJob || $obj instanceof ProcessBulkWhatsAppChunkJob) {
                return $obj;
            }
        } catch (Throwable) {
     ...
     */
    private function readChunkJobField(ProcessBulkSmsChunkJob|ProcessBulkWhatsAppChunkJob $job, string $name): mixed
    {
        try {
