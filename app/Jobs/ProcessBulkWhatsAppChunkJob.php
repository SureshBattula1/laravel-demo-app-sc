...
class ProcessBulkWhatsAppChunkJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        foreach ($this->recipientIds as $recipientId) {
            try {
                ...
                $to = $context['mobile'] ?? '';
                if ($to === '') {
                    Log::warning('Bulk WhatsApp skipped: no mobile', [
                ...
                $result = $dispatch->sendWhatsApp($this->branchId, $this->provider, $to, $body);
                $providerStatus = strtolower((string) ($result['status'] ?? ''));
                $finalSentStatuses = ['sent', 'delivered', 'read', 'received'];
                $finalFailedStatuses = ['failed', 'undelivered', 'canceled', 'cancelled'];
                $recipientStatus = in_array($providerStatus, $finalSentStatuses, true)
                    ? 'sent'
                    : (in_array($providerStatus, $finalFailedStatuses, true) ? 'failed' : 'pending');
                $errorMessage = $recipientStatus === 'failed'
                    ? ('Provider status: '.($providerStatus !== '' ? $providerStatus : 'failed'))
                    : null;
                $this->markRecipient($recipientId, $recipientStatus, $errorMessage, $result);
            } catch (\Throwable $e) {
                Log::warning('Bulk WhatsApp send failed for recipient', [
                    'branch_id' => $this->branchId,
                    'recipient_type' => $this->recipientType,
                    'id' => $recipientId,
                    'error' => $e->getMessage(),
